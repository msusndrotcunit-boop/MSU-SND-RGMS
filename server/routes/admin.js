        ...
        if (!studentId) {
            if (customUsername) {
                ...
                if (lastName && firstName) {
                    ...
                    // Set customUsername to First Name (as requested)
                    if (!customUsername) {
                         // Use First Name as temporary username
                         // This allows login via "First Name" (handled in auth.js)
                         // And also sets the initial username column to this value.
                         // If duplicates exist, auth.js handles it by asking for Student ID.
                         // If no Student ID, we might have issues, but we assume uniqueness for temp access or manual admin fix.
                         customUsername = firstName;
                    }
                }
            }
        }
...
const parsePdfBuffer = async (buffer) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // --- Special Handling for Split-Line Format (e.g. "CDT Agohob,\nMichael Jhon\nSulapas") ---
        // Check if current line looks like "Rank LastName" (with optional comma)
        const rankMatch = line.match(/^(CDT|Cdt|Cadet|Maj|Ltc|CPT|Cpt|2LT|1LT|SSg|Pvt)\s+([A-Za-z\s]+)(?:,)?$/i);
        
        // Check for Single-Line Rank format (e.g. "CDT Juan Dela Cruz" or "CDT Cruz, Juan")
        const singleLineRankMatch = line.match(/^(CDT|Cdt|Cadet|Maj|Ltc|CPT|Cpt|2LT|1LT|SSg|Pvt)\s+(.+)$/i);

        if (rankMatch) {
            const rank = rankMatch[1];
            const lastName = rankMatch[2].trim();
            let firstName = '';
            let middleName = '';
            ...
            if (i + 1 < lines.length) {
                ...
                // Heuristic: Next line is First Name if it's not another Rank/Header
                if (nextLine && !nextLine.match(/^(CDT|Cdt|Cadet|Maj|Ltc|CPT|Cpt|2LT|1LT|SSg|Pvt)\s+/i)) {
                     firstName = nextLine;
                     i++; // Consume this line

                     // Look ahead for Middle Name (e.g. "Sulapas" on next line)
                     if (i + 1 < lines.length) {
                         let nextNextLine = lines[i+1].trim();
                         // Heuristic: If it looks like a middle name (single word, capitalized)
                         // And NOT a School Name (University, College)
                         if (nextNextLine && 
                             !nextNextLine.match(/(University|College|School|Institute)/i) && 
                             nextNextLine.split(/\s+/).length <= 2) {
                             middleName = nextNextLine;
                             i++; // Consume this line
                         }
                     }
                }
            ...
            continue;
        } else if (singleLineRankMatch) {
             // Handle single line with Rank
             const rank = singleLineRankMatch[1];
             const fullName = singleLineRankMatch[2].trim();
             
             // Check if this is just a header or false positive (short text)
             if (fullName.length < 3) continue;

             let lastName = '';
             let firstName = '';
             let middleName = '';
             // let extractedCourse = ''; // User requested ONLY Rank/Name

             // Extract Course if present in the name line (Clean name but DO NOT import course)
             const lineCourseMatch = fullName.match(/\b(MS\s?\d{1,2}|COQC|NSTP\s?\d|NST\s?\d{3}|CWTS|LTS)\b/i);
             let nameOnly = fullName;
             if (lineCourseMatch) {
                 // extractedCourse = lineCourseMatch[0].toUpperCase().replace(/\s/g, '');
                 nameOnly = fullName.replace(lineCourseMatch[0], '').trim();
             }
             ...
             if (nameOnly.includes(',')) {
                 ...
                 if (restParts.length > 1) {
                     middleName = restParts.pop(); // Assume last word is middle name
                 ...
                 if (parts.length > 1) {
                     lastName = parts.pop();
                     firstName = parts.join(' '); // Assume rest is first name (Middle name hard to distinguish)
                 } else {
                     lastName = nameOnly;
                     firstName = 'Unknown';
                 }
             ...
             data.push({
                ...
                'Middle Name': middleName,
                'Email': '',
                'Cadet Course': '', // extractedCourse removed as per request
                'Course': '' 
            });
            continue;
        }
