...
const analyzeRotcmisExport = async (data, dayId) => {
    ...
    const cadetSeen = new Map();
    const existingByCadet = new Map();

    await new Promise((resolve, reject) => {
        db.all(
            'SELECT cadet_id, status, time_in, time_out FROM attendance_records WHERE training_day_id = ?',
            [dayId],
            (err, rows) => {
                if (err) reject(err);
                else {
                    for (const r of rows) {
                        existingByCadet.set(String(r.cadet_id), r);
                    }
                    resolve();
                }
            }
    ...
    try {
        ...
        console.error('ROTCMIS analyze: error fetching cadets', e);
    }
    ...
    for (let index = 0; index < rows.length; index++) {
        ...
        let qrValid = false;
        let qrStudentId = null;
        let qrUsername = null;

        if (qrPayloadRaw && typeof qrPayloadRaw === 'string') {
            try {
                const parsed = JSON.parse(qrPayloadRaw);
                if (parsed.student_id) qrStudentId = String(parsed.student_id);
                if (parsed.id && !qrStudentId) qrStudentId = String(parsed.id);
                if (parsed.Username) qrUsername = parsed.Username;
                qrValid = true;
            } catch (e) {
                qrInvalidCount++;
                issues.push(`Row ${index + 1}: invalid QR payload`);
            }
        }

        let cadet = null;
        try {
            cadet = await findCadet(row, allCadets);
        } catch (e) {
            issues.push(`Row ${index + 1}: error during cadet resolution`);
        }

        if (!cadet && qrStudentId) {
            try {
                const byId = await getCadetByStudentId(qrStudentId);
                if (byId) cadet = byId;
            } catch (e) {}
        }
        ...
        if (!cadet && qrUsername) {
            try {
                const byUser = await getCadetByUsername(qrUsername);
                if (byUser) cadet = byUser;
            } catch (e) {}
        }
        ...
        if (!cadet) {
            ...
            previewRecords.push({
                ...
                time_out:
                    ...
                    null,
                cadetId: null,
                studentId: row['Student ID'] || row['ID'] || row['Student Number'] || row['student_id'] || null,
                name:
                    row['Name'] ||
                    row['Cadet Name'] ||
                    row['CADET NAME'] ||
                ...
                validation: {
                    isDuplicate: false,
                    hasConflict: false,
                    issues: ['Cadet not found in system']
                }
            });
            continue;
        }
        ...
        const cadetIdKey = String(cadet.id);
        const seen = cadetSeen.get(cadetIdKey) || 0;
        cadetSeen.set(cadetIdKey, seen + 1);
        ...
        const existing = existingByCadet.get(cadetIdKey) || null;
        ...
        if (existing) {
            if (existing.status && existing.status !== status) {
                hasConflict = true;
                recordIssues.push('Status differs from existing record');
            }
            if (time_in && existing.time_in && existing.time_in !== time_in) {
                hasConflict = true;
                recordIssues.push('Time in differs from existing record');
            }
            if (time_out && existing.time_out && existing.time_out !== time_out) {
                hasConflict = true;
                recordIssues.push('Time out differs from existing record');
            }
        }

        if (qrPayloadRaw && !qrValid) {
            recordIssues.push('QR payload invalid');
        }

        if (!qrPayloadRaw) {
            recordIssues.push('No QR payload provided');
        }
        ...
        previewRecords.push({
            index,
            canApply: !hasConflict,
            reason: hasConflict ? 'conflict' : isDuplicate ? 'duplicate' : 'ok',
            status,
            remarks,
            time_in,
            time_out,
    ...
    const baseScore = (mappable / denominator) * 100;
    const penalty = (conflictCount + qrInvalidCount) * 2;
    const dataQualityScore = Math.max(0, Math.min(100, Math.round(baseScore - penalty)));
...
router.post('/import-rotcmis-preview', authenticateToken, isAdmin, upload.single('file'), async (req, res) => {
    if (!req.file) return res.status(400).json({ message: 'No file uploaded' });
    const { dayId } = req.body;
    if (!dayId) return res.status(400).json({ message: 'Day ID is required' });
    ...
    try {
        ...
        if (filename.endsWith('.xlsx') || filename.endsWith('.xls') || filename.endsWith('.csv')) {
            ...
            const rawRows = await parseDocx(buffer);
            data = rawRows.map(r => extractFromRaw(r.raw));
        } else if (filename.match(/\.(png|jpg|jpeg|bmp|webp|gif|tiff)$/i)) {
            if (!buffer) throw new Error('File buffer missing for Image import');
            const rawRows = await parseImage(buffer);
            data = rawRows.map(r => extractFromRaw(r.raw));
        } else {
        ...
        const analysis = await analyzeRotcmisExport(data, dayId);
        ...
        res.json({
            ...
            summary: {
                ...
                qrInvalidCount: analysis.qrInvalidCount,
                dataQualityScore: analysis.dataQualityScore
            },
            previewRecords: analysis.previewRecords,
            issues: analysis.issues
        });
    } catch (error) {
        console.error('ROTCMIS preview error:', error);
        res.status(500).json({ message: 'Failed to generate preview' });
    }
});

router.post('/import-rotcmis-commit', authenticateToken, isAdmin, async (req, res) => {
    const { dayId, records } = req.body || {};
    if (!dayId) return res.status(400).json({ message: 'Day ID is required' });
    ...
    for (const r of records) {
        if (!r || !r.cadetId) {
            failCount++;
            errors.push('Record missing cadetId');
            continue;
        }
        const status = r.status || 'present';
        const remarks = r.remarks || '';
        const time_in = r.time_in || null;
        const time_out = r.time_out || null;

        try {
            await upsertAttendance(dayId, r.cadetId, status, remarks, time_in, time_out);
            successCount++;
        } catch (e) {
            failCount++;
            errors.push(`Failed for cadet ${r.cadetId}: ${e.message}`);
        }
    }

    res.json({
        message: `ROTCMIS import applied. Success: ${successCount}, Failed: ${failCount}`,
        summary: {
            successCount,
            failCount,
...
// Scan Staff QR Code
// Removed duplicate staff scan route in favor of the earlier implementation that supports providedStatus
