...
- Timing and easing:
  - Default interaction duration: `180ms`
  - Easing: `cubic-bezier(0.4, 0, 0.2, 1)` for hover and focus transitions
- Elevation on hover (`.hover-highlight`):
  - Base: no extra shadow
  - Hover:
    - `box-shadow: 0 4px 16px rgba(15, 23, 42, 0.16)`
    - `transform: translateY(-1px)`
    - `background-color: rgba(255, 255, 255, 0.96)` on light backgrounds
- Icon emphasis (`.hover-icon-highlight`):
  - Transition: color and transform
  - Hover:
    - `color: var(--primary-color)` (`#354f32`)
    - Slight lift: `translateY(-0.5px)`
- Text emphasis (`.hover-text-highlight`):
  - Transition: color only
  - Hover color: `var(--primary-color)`
- Recommended usage:
  - Sidebar navigation items: wrap nav rows with `hover-highlight`
  - Primary action buttons: Tailwind background classes plus `hover-highlight`
  - Status cards and summary pills: base `bg-*-50` with `hover-highlight` for lift
  - Icon buttons in top bars: combine `hover-icon-highlight` with subtle background (`hover:bg-gray-100`)
## Alignment Patterns
